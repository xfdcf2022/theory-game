...
// 三学派引擎钩子 + 遗物 + 意图系统
export class Game {
  constructor({ school, deckCards, enemy, relics = [] }) {
    this.school = school;
  ...
  start() {
    ...
    const drawRelic = this.relics.find((r) => r.effect.drawOnStart);
    if (drawRelic) {
  ...
  playCard(card) {
    ...
    if (card.cost > this.player.energy) return { ok: false, reason: '灵感不足' };
  ...
  endTurn() {
    ...
    if (this.school === 'zheng' && this.player.energy > 0) {
      const save = Math.floor(this.player.energy / 2);
